...
class DecisionTreeOld(maxDepth: Int = 3, featureSubsetStrategy: String = "all", minSplitSize: Int = 5, impurityMeasure: String = "gini", par: Boolean = false) extends Serializable {
  ...
  def train(data: RDD[DataPoint], weights: Option[Seq[Double]] = None): Unit = {
    ...
    val w = weights match {
      case Some(x) => x
      case None => List.fill(data.collect().length)(1.0)
    }

    val impurityFunc = getImpurityMeasure
    root = Some(buildDecisionTree(data_filtered, w, impurityFunc, maxDepth, minSplitSize))
  }
  ...
  def buildDecisionTree(data: RDD[DataPoint],
    ...
    if (maxDepth == 0 || labels.length < minSplitSize) {
      Leaf(getMajorityWeighted(labels, weights))
    } else {
      //val ((bestFeature, bestSplits, bestGain), timeIn) = time("pippo", getSelectorFeatures()(data, weights, impurityFunc))
  ...
  def findBestWeightedSplit(
    ...
    for (((value, _), weight) <- data.dropRight(1)) {
      leftWeight += weight
      leftCount += 1
      rightWeight -= weight
      rightCount -= 1
