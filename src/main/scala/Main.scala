...
@main def Main(classifier: String, master: String, dataset: String, parallelism: Int, limit: Boolean, simulation: Boolean): Unit = {
  ...
  if (simulation) {
    ...
    val decisionTree = DecisionTreeOld(par = true)
    ...
    val decisionTreeSeq = DecisionTreeOld(par = false)
    ...
    val randomForest = RandomForest(par = true)
    ...
    val randomForestSeq = RandomForest(par = false)
